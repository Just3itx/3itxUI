...
function registerLuauLanguage(monaco: Monaco) {
    ...
    monaco.languages.setMonarchTokensProvider("luau", {
        ...
        exploitGlobals: [
            ...
            "firesignal", "replicatesignal", "fireclickdetector", "fireproximityprompt",
            "firetouchinterest", "fireserver", "hookfunction", "hookmetamethod",
            "newcclosure", "iscclosure", "islclosure", "clonefunction", "getinfo",
            "checkcaller", "checkclosure", "isexecutorclosure", "getnamecallmethod",
            ...
            "decompile", "saveinstance", "getthreadidentity", "setthreadidentity",
            "crypt", "base64",
        ],
...
const KNOWN_GLOBALS = new Set([
    ...
    "request", "http_request", "syn", "fluxus", "KRNL_LOADED",
    "decompile", "saveinstance", "getthreadidentity", "setthreadidentity",
    "crypt", "base64", "Drawing",
    "_G", "_VERSION", "shared",
]);

/** Lightweight Luau diagnostics: check brackets, strings, block keywords */
function validateLuau(
    ...
    // Track block openers for end/until matching
    ...
    for (let ln = 1; ln <= lineCount; ln++) {
        ...
        let line = raw;
        // Remove multiline comment indicators (simplified)
        line = line.replace(/--\[\[.*?\]\]/g, "");
        ...
        // Check for unclosed strings in the raw line
        const rawNoComment = raw.replace(/--.*$/, "");
        let inSingle = false, inDouble = false;
        for (let i = 0; i < rawNoComment.length; i++) {
            const ch = rawNoComment[i];
            if (ch === "\\") { i++; continue; }
            if (ch === '"' && !inSingle) inDouble = !inDouble;
            if (ch === "'" && !inDouble) inSingle = !inSingle;
        }
        ...
        const words = line.match(/\b(function|if|for|while|do|repeat|end|until|elseif|else)\b/g);
        if (words) {
            for (const w of words) {
                if (w === "function" || w === "if" || w === "for" || w === "while" || w === "do") {
                    blockStack.push({ keyword: w, line: ln });
                } else if (w === "repeat") {
                    blockStack.push({ keyword: "repeat", line: ln });
                } else if (w === "until") {
                    if (blockStack.length > 0 && blockStack[blockStack.length - 1].keyword === "repeat") {
                        blockStack.pop();
                    } else {
                        markers.push({
                            severity: monaco.MarkerSeverity.Error,
                ...
                // elseif/else don't change the stack
            }
        }
    }

    // Report unclosed blocks at end of file
    for (const b of blockStack) {
        markers.push({
            severity: monaco.MarkerSeverity.Error,
            message: `'${b.keyword}' on line ${b.line} has no matching '${b.keyword === "repeat" ? "until" : "end"}'`,
            startLineNumber: b.line, endLineNumber: b.line,
            startColumn: 1, endColumn: model.getLineContent(b.line).length + 1,
        });
    }
