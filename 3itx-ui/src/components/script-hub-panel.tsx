...
import {
    ...
    TrendingUp, Clock, Flame, Copy, FileEdit,
} from "lucide-react";
...
interface ScriptHubPanelProps {
    ...
    onInsertToEditor?: (title: string, content: string) => void;
}
...
/* ─── Component ─── */
export default function ScriptHubPanel({ onLoad, onInsertToEditor }: ScriptHubPanelProps) {
    ...
    const [contextMenu, setContextMenu] = useState<{ x: number; y: number; script: NormalizedScript } | null>(null);
        ...
        <div className="h-full overflow-auto">
            ...
            {contextMenu && (
                <>
                    ...
                    />
                    <div
                        className="fixed z-[999] min-w-[160px] py-1 bg-[#1a1a1e] border border-white/[0.1] rounded-lg shadow-xl"
                        style={{ left: contextMenu.x, top: contextMenu.y }}
                    >
                        <button
                            onClick={() => {
                                if (onInsertToEditor) onInsertToEditor(contextMenu.script.title, contextMenu.script.script);
                                setContextMenu(null);
                            }}
                            className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] text-foreground hover:bg-white/[0.06] transition-colors"
                        >
                        ...
                        <button
                            onClick={() => {
                                navigator.clipboard.writeText(contextMenu.script.script);
                                setContextMenu(null);
                            }}
                            className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] text-foreground hover:bg-white/[0.06] transition-colors"
                        >
