...
local function enableConsoleRedirect()
    ...
    if hookfunction and env.print then
        _origPrint = hookfunction(env.print, function(...)
            sendLog("info", ...)
        end)
    end

    -- Hook warn — redirect only to UI, suppress Roblox console
    if hookfunction and env.warn then
        _origWarn = hookfunction(env.warn, function(...)
            sendLog("warning", ...)
        end)
    end

    -- Hook error — send to UI then still throw (error controls flow)
    if hookfunction and env.error then
        _origError = hookfunction(env.error, function(msg, ...)
            sendLog("error", msg)
...
local function disableConsoleRedirect()
    ...
    local env = getfenv()
    if _origPrint and hookfunction then
        pcall(function() hookfunction(env.print, _origPrint) end)
        _origPrint = nil
    end
    if _origWarn and hookfunction then
        pcall(function() hookfunction(env.warn, _origWarn) end)
        _origWarn = nil
    end
    if _origError and hookfunction then
        pcall(function() hookfunction(env.error, _origError) end)
        _origError = nil
    end
end
...
local function enableLSP()
    ...
    task.spawn(function()
        ...
        local workspaceConn = game:GetService("Workspace").DescendantAdded:Connect(function(inst)
        ...
        local removeConn = game:GetService("Workspace").DescendantRemoving:Connect(function(inst)
            if not _lspEnabled then return end
            task.defer(function()
                wsSend({
                    type = "lsp_remove",
